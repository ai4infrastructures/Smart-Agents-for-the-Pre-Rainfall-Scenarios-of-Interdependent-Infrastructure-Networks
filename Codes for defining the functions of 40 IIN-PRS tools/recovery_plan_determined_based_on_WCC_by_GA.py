...
def recovery_plan_determined_based_on_WCC_by_GA(global_json_path):
    global_json_path = global_json_path.strip().replace('\n', '')

    with open(global_json_path, 'r') as f:
        global_data = json.load(f)

    network_path = global_data["resource_constrained_interdependent_critical_infrastructures_networks"]
    resource_constraints_path = global_data["resource_constraints_per_day"]
    cascading_failure_path = global_data["failure_node_after_HECRAS_simulations"]
    ...
    with open(cascading_failure_path, 'r') as f:
        failed_nodes = json.load(f)["all_failed_nodes"]
    with open(population_data_path, 'r') as f:
        population_data = json.load(f)

    resource_types = resource_constraints['repair_teams'].keys()

    def build_network(active_nodes):
        G = nx.Graph()
        for node in network_data['nodes']:
            if node['Code'] in active_nodes:
                G.add_node(node['Code'])
        for edge in network_data['edges']:
            if edge['Start'] in active_nodes and edge['End'] in active_nodes:
    ...
    def fitness(order):
        ...
        while remaining_order or node_progress:
            available_resources = {
                rtype: resource_constraints['repair_teams'][rtype]['resource_per_day'] for rtype in resource_types
            }
            recovered_today = []
            ...
            for node in remaining_order[:]:
                ...
                remaining_resources = node_progress.get(node, {
                    rtype: node_data.get(rtype, 0) for rtype in resource_types
                })
                for rtype in resource_types:
                    allocation = min(remaining_resources[rtype], available_resources[rtype])
                    available_resources[rtype] -= allocation
                    remaining_resources[rtype] -= allocation
            ...
            recovered_nodes.update(recovered_today)
            G = build_network(recovered_nodes)
            WCC = len(max(nx.connected_components(G), key=len)) if len(G.nodes) > 0 else 0
            WCC_total += WCC

        return -WCC_total  # maximize WCC sum over time
    ...
    while best_order or node_progress:
        ...
        recovered_nodes.update(nodes_recovered_today)
        G = build_network(recovered_nodes)
        WCC = len(max(nx.connected_components(G), key=len)) if len(G.nodes) > 0 else 0
    ...
    output_json_path = 'recovery_plan_determined_based_on_WCC_by_GA.json'
    with open(output_json_path, 'w') as f:
        json.dump(daily_recovery_order, f, indent=4)

    global_data["recovery_plan_determined_based_on_WCC_by_GA"] = output_json_path
...
recovery_plan_determined_based_on_WCC_by_GA(global_json_path)