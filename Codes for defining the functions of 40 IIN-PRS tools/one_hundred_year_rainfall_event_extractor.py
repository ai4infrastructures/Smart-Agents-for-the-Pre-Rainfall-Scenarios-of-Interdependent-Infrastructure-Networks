...
def one_hundred_year_rainfall_event_extractor(global_json_path):
    # 定义输出文件名称
    output_path = "one_hundred_year_rainfall_event.json"
    ...
    try:
        ...
        print(f"The file {output_path} was not found. Create a new empty data file")
        event_data = {}
    ...
    global_data["one_hundred_year_rainfall_event"] = output_path

    # 写回更新后的全局数据文件
    with open(global_json_path, 'w', encoding='utf-8') as global_file:
        json.dump(global_data, global_file, indent=4, ensure_ascii=False)

    print(f"The global data file has been updated：{global_json_path}")


# 示例用法
global_json_path = "Global_Data.json"
one_hundred_year_rainfall_event_extractor(global_json_path)
