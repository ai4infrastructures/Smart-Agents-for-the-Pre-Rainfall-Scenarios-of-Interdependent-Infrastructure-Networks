...
def shapefile_network_converter(json_input_path: str)-> str:
    # 读取输入的JSON文件
    ...
    infrastructure_information = data['infrastructures_information']
    ...
    # 将合并后的网络数据保存到JSON文件
    output_json_path = 'infrastructures_networks.json'
    with open(output_json_path, 'w') as outfile:
        json.dump(all_network_data, outfile, indent=4)

    with open(json_input_path, 'r') as file:
        global_data = json.load(file)
    global_data['infrastructures_networks'] = output_json_path

    with open(json_input_path, 'w') as file:
        json.dump(global_data, file, indent=4)
        return "The path to infrastructures_networks has been saved in Global_data.json"

# 调用函数
shapefile_network_converter('Global_Data.json')
