...
def network_generator_for_interdependent_critical_infrastructures(input_file):
    ...
    with open('interdependent_critical_infrastructures_networks.json', 'w') as outfile:
        json.dump(json_data, outfile, indent=4)

    # Update the Global_Data.json to store the path to the new file
    data['interdependent_critical_infrastructures_networks'] = 'interdependent_critical_infrastructures_networks.json'
    with open(input_file, 'w') as outfile:
        json.dump(data, outfile, indent=4)
    return "The path to interdependent_critical_infrastructures has been saved in Global_data.json"
    print("The modified network has been saved to interdependent_critical_infrastructures.json.")

# Example usage:
modified_network = network_generator_for_interdependent_critical_infrastructures('Global_Data.json')
