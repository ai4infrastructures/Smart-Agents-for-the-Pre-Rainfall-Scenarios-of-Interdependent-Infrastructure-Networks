...
def post_disaster_assessment_based_on_connectivity(global_json_path: str):
    ...
    # Calculate the size of the connectivity in the original network
    connectivity_before = nx.node_connectivity(G.to_undirected())

    with open(failure_file, 'r') as f:
        failure_data = json.load(f)
    initial_nodes = failure_data.get("all_failed_nodes", [])
    initially_failed_nodes = set(initial_nodes)
    ...
    # Calculate the size of the connectivity after cascading failures
    if G_after_failure.number_of_nodes() >= 2:
        connectivity_after  = nx.node_connectivity(G_after_failure.to_undirected())
    else:
    ...
    result = {
        ...
        'connectivity_before': connectivity_before,  # connectivity of nodes before failure
        'connectivity_after': connectivity_after,    # connectivity of nodes after failure
        'network_resilience': network_resilience, # Network resilience ratio
    }

    output_json_path = 'post_disaster_assessment_based_on_connectivity.json'
    ...
    file_paths['post_disaster_assessment_based_on_connectivity'] = output_json_path
...
post_disaster_assessment_based_on_connectivity(global_json_path)
