...
3. Before all actions begin, you need to first plan the overall execution steps to complete the task. A suggested overall execution steps for solving the tasks is:
{suggested_steps}
Begin!
Thought: {agent_scratchpad}"""
)

# Register all Tool objects by the variable names written in LLM_results.xlsx.
# For example: "shapefile_network_converter_tool" -> shapefile_network_converter_tool
# The complete list is retained only as a registry; each task receives only its selected tools.
tool_registry = {
    f"{tool.name}_tool": tool
    for tool in all_tools
}


def normalize_question(value):
    """Normalize whitespace and letter case so questions can be matched reliably."""
    return " ".join(str(value).split()).casefold()


def parse_selected_tools(tool_chain_value):
    """Convert the newline-separated Results_1 cell into LangChain Tool objects."""
    if pd.isna(tool_chain_value) or not str(tool_chain_value).strip():
        raise ValueError("The Results_1 tool chain is empty.")

    selected_tool_names = [
        line.strip().rstrip(",")
        for line in str(tool_chain_value).replace("\r\n", "\n").split("\n")
        if line.strip()
    ]
    ...
    if unknown_tool_names:
        raise ValueError(
            "Unknown tool name(s) in Results_1: "
            + ", ".join(unknown_tool_names)
        )

    return [tool_registry[tool_name] for tool_name in selected_tool_names]


def format_suggested_steps(step_value):
    """Format the Results_step cell for insertion into the task-specific prompt."""
    if pd.isna(step_value) or not str(step_value).strip():
        raise ValueError("The Results_step execution plan is empty.")

    step_lines = [
        line.strip()
        for line in str(step_value).replace("\r\n", "\n").split("\n")
        if line.strip()
    ]
    return "\n".join(f"    {line}" for line in step_lines)


# Read the task file and the TPP-generated tool chains and suggested steps.
df = pd.read_excel(r"C:\Users\26389\OneDrive\Desktop\Task.xlsx")
llm_results_df = pd.read_excel(
    r"C:\Users\26389\OneDrive\Desktop\tool\LLM_results.xlsx"
)

required_columns = {"question", "Results_1", "Results_step"}
missing_columns = required_columns - set(llm_results_df.columns)
if missing_columns:
    raise KeyError(
        "Missing required column(s) in LLM_results.xlsx: "
        + ", ".join(sorted(missing_columns))
    )

# Match each task to its selected tool chain and suggested steps by question.
question_to_plan = {}
for _, result_row in llm_results_df.iterrows():
    question_key = normalize_question(result_row["question"])
    if question_key and question_key not in question_to_plan:
        question_to_plan[question_key] = {
            "tools": result_row["Results_1"],
            "steps": result_row["Results_step"]
        }
...
for index, row in df.iterrows():
    ...
    try:
        # Read the tool chain and suggested execution steps for the current task.
        question_key = normalize_question(question)
        if question_key not in question_to_plan:
            raise ValueError(
                "No matching question was found in LLM_results.xlsx: "
                + str(question)
            )

        plan_data = question_to_plan[question_key]
        tools = parse_selected_tools(plan_data["tools"])
        suggested_steps = format_suggested_steps(plan_data["steps"])

        # Insert the current task's suggested steps into its prompt.
        task_prompt_template = prompt_template.partial(
...
output_file = r"C:\Users\26389\OneDrive\Desktop\KGP-ReAct-response.xlsx"
