...
# Read the task file and the TPP-generated tool chains.
...
required_columns = {"question", "Results_1"}
...
# Match each task to its selected tool chain by question rather than row number.
question_to_tool_chain = {}
for _, result_row in llm_results_df.iterrows():
    question_key = normalize_question(result_row["question"])
    if question_key and question_key not in question_to_tool_chain:
        question_to_tool_chain[question_key] = result_row["Results_1"]
...
for index, row in df.iterrows():
    ...
    try:
        # Read the tool chain selected for the current task from LLM_results.xlsx.
        question_key = normalize_question(question)
        if question_key not in question_to_tool_chain:
            raise ValueError(
                "No matching question was found in LLM_results.xlsx: "
                + str(question)
            )

        tools = parse_selected_tools(question_to_tool_chain[question_key])
...
output_file = r"C:\Users\26389\OneDrive\Desktop\TPP-ReAct-response.xlsx"
